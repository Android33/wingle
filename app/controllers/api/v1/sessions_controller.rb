class Api::V1::SessionsController < ApplicationController
  ...
  def create
    ...
    if token.present?
      ...
      if user.userinfo
        info = user.userinfo
        render json: {STATUS_MSG: "USER_INFO_FOUND", STATUS_CODE: OK_STATUS_CODE, user_token: token, user_email: email, name: user.name, image_id: user.image_id, gender: info.gender, height: info.height,
